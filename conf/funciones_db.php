...
function vista_profesores($id_establecimiento){
    ...
    $docente_run =  user_docente(trim($fila["run_docente"]));
           ...
           .'<td><button id="actualiza_docente" class="btn btn-danger actualiza_docente"  data-toggle="modal" data-target="#modal_actualizar_docente">Editar</button></td></tr>';  
        ...
        if ($contador != 0) {
            while ($row = $resultado->fetch(PDO::FETCH_ASSOC)) {
                ...
                echo '<div class="col-md-3">'
                    ...
                    . '<div class="card-body">'
                   . '<a class="btn btn-success" href="eliminar_datos.php?curso=' . $row['id_curso'] .'&docente='. $row['id_docente'] . '&establecimiento=' . $row['id_establecimiento'] .'" data-toggle="tooltip" data-placement="top" title="Ver Curso">Curso: ' . $row["curso"]
                    . '</a>'
...
function nuevo_estudiante_uni($nom_estu,$apelli_estu,$run_estu,$fech_naci_estu,$token_estu,$ciu_estu,$id_establecimiento,$id_docente,$id_curso,$id_nivel,$id_pais)
{
    try{
        date_default_timezone_set('America/Santiago'); 
     ...
     //   $fk_nivel = nivel_estudiante_establecimiento($id_establecimiento,$id_docente,$id_curso);          
               ...
               VALUES (:estado_encuesta,:nom_estu,:apelli_estu,:run_estu,:fech_naci_estu,:fecha_registro,:token_estu,:ciu_estu,:id_establecimiento,:id_docente,:id_curso,:id_nivel,:id_pais)");
               $query->execute([
...
try{
    $conexion =connectDB_demos();  
    $query = $conexion->prepare("SELECT ce_participantes_run FROM ce_participantes WHERE ce_participantes_run= :ce_participantes_run ");
    $query->execute([":ce_participantes_run"=>$run]);
    $conexion = NULL;
    $resultado = $query->rowCount();
...
function select_establecimiento($id_establecimiento)
{
    ...
    $con = connectDB_demos();
    $query = $con->query("SELECT c.id_ce_docente as id_docente,c.ce_docente_nombres as nom_docente, c.ce_docente_apellidos as apelli_docente
    FROM ce_estable_curso_docente a
    INNER JOIN ce_establecimiento b ON a.ce_fk_establecimiento = b.id_ce_establecimiento
    INNER JOIN ce_docente c ON a.ce_fk_docente = c.id_ce_docente
    INNER JOIN ce_curso d ON a.ce_fk_curso = d.id_ce_curso
...
{
    $con = connectDB_demos();
    $query = $con->query("SELECT d.id_ce_curso as id_curso,d.ce_curso_nombre AS nom_curso, a.ce_fk_nivel AS id_nivel, e.ce_nombre AS nom_nivel
    FROM ce_estable_curso_docente a
   INNER JOIN ce_establecimiento b ON a.ce_fk_establecimiento = b.id_ce_establecimiento
   INNER JOIN ce_docente c ON a.ce_fk_docente = c.id_ce_docente
   INNER JOIN ce_curso d ON a.ce_fk_curso = d.id_ce_curso
...
function valida_carga_excel($id_establecimiento, $id_docente, $id_curso)
{
    try{
        ...
        $query = $con->query("SELECT *
         ...
         a.ce_curso_id_ce_curso ='$id_curso'");
          $con = null;
          $resultado = $query->fetch(PDO::FETCH_ASSOC);
          return $resultado;
...
function guarda_establecimientos($nombre,$rbd,$id_pais)
{
    try{
        ...
        WHERE a.ce_establecimiento_nombre ='$nombre' OR a.ce_establecimiento_rbd ='$rbd'");
        $resultado = $query->fetch(PDO::FETCH_ASSOC);
                          ...
                          $num41, $num42, $num43, $num44, $num45, $num46, $num47)
{
    try{
        ...
        ce_p41,ce_p42,ce_p43,ce_p44,ce_p45,ce_p46,ce_p47) 
...
function dimension_afectivo_curso_copia($establecimiento, $profesor)
{
    try {
        #pregunta número 1
        $suma_1_p1 = 0;
...
function factores_contextuales_estudiante($token_estudiante, $id_docente)
{
    try {
